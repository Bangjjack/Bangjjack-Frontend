...
function RoundButton({ className, label = "룸메이트 모집하기", ...props }: RoundButtonProps) {
  return (
    <button
      type="button"
      className={cn(
        "fixed bottom-[107px] left-1/2 z-50 -translate-x-1/2 flex cursor-pointer items-center gap-[10px] rounded-full bg-button-primary px-400 py-200 shadow-[0px_3px_12px_0px_rgba(0,0,0,0.24)] hover:brightness-95 active:bg-button-primary-pressed",
        className,
      )}
      {...props}
    >
