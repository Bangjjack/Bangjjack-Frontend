...
**If the task involves building or updating a full page, screen, or multi-section layout in Figma from code**, also load **figma-generate-design** when it is available (same plugin skill pack; add it under `.cursor/skills/` if you copy it from `~/.cursor/plugins/.../skills/`). It covers `search_design_system`, imports, and incremental screen assembly. This skill covers Plugin API rules; that one covers the screen-building workflow.
